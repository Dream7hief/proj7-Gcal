...
def get_busy_times():
    calendars = request.args.get("allChecked", type=str)
    calendars = list(calendars.split(','))
    ...
    for calID in calendars:
        events = gcal_service.events().list(calendarId=calID,timeMin=start_datetime, timeMax=end_datetime).execute()
        modified_events = get_busy_events(events, start_time, end_time)
        ...
        for i in range(len(modified_events)):
            ...
            flask.flash(title)
            flask.flash(humanize(start))
            flask.flash(humanize(end))
...
def setrange():
    ...
    flask.session['startTime'] = startTime
...
def get_datetime(date,time):
    date = date.strip() #strips whitespace
    date = interpret_date(date) #Reformats date to correct style for arrow
    date = arrow.get(date).format("YYYY-MM-DD") #gets just the date of the arrow representation

    time = interpret_time(time)#Reformats time to correct style for arrow
    time = arrow.get(time).format("HH:mm") #gets just the time of the arrow representation
    datetime = arrow.get(date + ' ' + time).replace(tzinfo=tz.tzlocal()).isoformat() #combines the two times and dates
    return datetime

def get_busy_events(events, start_time, end_time):
    modified_events = []
    for i in range(len(events['items'])):
        ...
        interpretted_start_time = interpret_time(start_time)
        interpretted_end_time = interpret_time(end_time)
        if (start >= interpretted_start_time):
            if (end <= interpretted_end_time):
